...
interface RecoveryRecord { version: 1; notes: NoteChange[]; patches: SettingsPatch[] }
  ...
  async request(change: PropertyMigration, configure: (settings: typeof this.plugin.settings) => void): Promise<boolean> {
    ...
    try {
      ...
      const confirmed = await PropertyMigrationModal.confirm(this.plugin.app, name,
        `${plan.notes.length} Markdown notes will change, including archived notes and templates. This updates frontmatter only. Exact string values and list items match; note bodies, inline fields, Base formulas, and other plugins’ settings are not rewritten. ${patches.length} GCM settings groups will update. A temporary local recovery copy is kept until completion.`,
      ...
      const record: RecoveryRecord = { version: 1, notes: plan.notes, patches };
      this.active = true;
